...
class PairsTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if (self.zscore[0] > self.upper_limit) and (self.status != 1):
            ...
            print('SELL CREATE %s, price = %.2f, qty = %d' % ("WTI", self.data0.close[0], x + self.qty1))
            self.sell(data=self.data0, size=(x + self.qty1))  # Place an order for buying x + qty1 shares
            print('BUY CREATE %s, price = %.2f, qty = %d' % ("BRNT", self.data1.close[0], y + self.qty2))
            ...
            # Place the order
            # print('zscore is', self.zscore[0])
            print('BUY CREATE %s, price = %.2f, qty = %d' % ("WTI", self.data0.close[0], x + self.qty1))
            self.buy(data=self.data0, size=(x + self.qty1))  # Place an order for buying x + qty1 shares
            print('SELL CREATE %s, price = %.2f, qty = %d' % ("BRNT", self.data1.close[0], y + self.qty2))
            ...
            if order1 is not None:
                print('CLOSE POSITION %s, price = %.2f' % ("WTI", self.data0.close[0]))
            if order2 is not None:
                print('CLOSE POSITION %s, price = %.2f' % ("BRNT", self.data1.close[0]))
